...
GitRemoteStatus.prototype = {
    ...
    perform: function(config, req) {
        ...
        shell.exec_dir('git remote -v | grep fetch | cut -f2 | cut -d" " -f1', config.dir).then(function(remote) {
            shell.exec_dir('git rev-parse --abbrev-ref HEAD', config.dir).then(function(local_branch) {
                shell.exec_dir('git ls-remote '+remote+' | grep "refs/heads/'+local_branch+'$" | cut -f1', config.dir).then(function(remote_head) {
